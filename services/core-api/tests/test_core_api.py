...
entry = next(p for p in core_dir.glob("*.py") if p.stem != "state")
spec = importlib.util.spec_from_file_location(
    "core", entry, submodule_search_locations=[str(core_dir)]
)
...
def test_palette_ownership():
    ...
    with SessionLocal() as db:
        db.execute(
            sa.insert(palette_purchases).values(
                user_id=u1, palette_key="midnight_rose", price_cents=300
            )
        )
        db.commit()
    set_session(u1)
