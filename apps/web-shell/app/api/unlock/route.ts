...
if (!globalThis.__unlockRate)
  globalThis.__unlockRate = new Map<string, RateRecord>();
const rateMap: Map<string, RateRecord> = globalThis.__unlockRate as Map<
  string,
  RateRecord
>;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (ok) {
      // reset on success
      rateMap.delete(ip);
      // keep response minimal and compatible; include optional redirect
