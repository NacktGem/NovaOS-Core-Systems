...
class GlitchAgent(BaseAgent):
    ...
    def assess_threat_level(self, finding_type: str, details: Dict[str, Any]) -> str:
        ...
        if any(indicator in finding_type.lower() or indicator in str(details).lower() 
               for indicator in critical_indicators):
            return "critical"
        elif any(indicator in finding_type.lower() or indicator in str(details).lower() 
    ...
    def _hash_file(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if suspicious_indicators:
            self.log_finding("suspicious_file", {
                "path": str(path),
    ...
    def _scan_system(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for proc in processes[:20]:  # Check first 20 processes
                if any(suspicious in proc.lower() for suspicious in 
                       ['nc ', 'ncat', 'socat', '/tmp/', 'wget http', 'curl http']):
            ...
            # Log anomalies
            if anomalies:
                self.log_finding("system_anomalies", {
                    "scan_id": scan_id,
                    "anomalies": anomalies
                })
    ...
    def _detect_entropy(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if entropy > 7.5:
            ...
            analysis["indicators"] = ["possible_encryption", "possible_compression", "possible_malware"]
        elif entropy > 6.5:
        ...
        if analysis["assessment"] != "normal":
            self.log_finding("entropy_anomaly", {
                "path": str(path),
                "entropy": entropy,
    ...
    def _sandbox_check(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            indicators["vm_cpu"] = any(vm in cpuinfo for vm in ["vmware", "virtualbox", "qemu", "xen"])
        except Exception:
        ...
        virtualized = (
            "docker" in indicators["cgroup"] or 
            indicators["container_env"] or
            indicators["vm_cpu"] or 
            indicators["vm_dmi"]
        )
        ...
        result = {
            ...
            "confidence": "high" if sandbox_score >= 2 else "medium" if sandbox_score == 1 else "low"
        }
    ...
    def _network_probe(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = {
            ...
            "risk_level": "high" if suspicious_services else "medium" if len(open_ports) > 10 else "low"
        }
        
        # Log suspicious findings
        if suspicious_services or len(open_ports) > 15:
            self.log_finding("network_anomaly", {
                "host": host,
    ...
    def _detect_suspicious_patterns(self, data: bytes) -> List[str]:
        ...
        suspicious_strings = [
            'password', 'admin', 'backdoor', 'rootkit', 'keylog',
            'http://', 'https://', 'ftp://', '.exe', '.dll', '.bat'
        ]
        
        for sus_str in suspicious_strings:
            if sus_str in data_str:
                patterns.append(f"contains_{sus_str.replace('://', '_protocol').replace('.', '_ext')}")
        ...
        try:
            ...
            for proc_line in processes:
                ...
                if len(parts) >= 7:
                    ...
                    if any(sus in comm.lower() or sus in proc_line.lower() for sus in
                           ['/tmp/', 'nc', 'ncat', 'socat', 'wget', 'curl http']):
    ...
    def _analyze_logs(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for log_path in log_paths:
            try:
                ...
                if any(pattern in log_content.lower() for pattern in 
                ...
                if any(pattern in log_content.lower() for pattern in
                       ['wget', 'curl', 'nc -', 'socat', '/tmp/', 'chmod +x']):
        ...
        # Summary
        total_suspicious = sum(len(result.get("suspicious_patterns", [])) for result in analysis_results)
        
        result = {
            "logs_analyzed": len(analysis_results),
            "analysis_results": analysis_results,
            "total_suspicious_patterns": total_suspicious,
            "threat_assessment": "high" if total_suspicious > 5 else "medium" if total_suspicious > 0 else "low"
        }
        
        if total_suspicious > 0:
            self.log_finding("log_anomalies", {
                "suspicious_patterns": total_suspicious,
                "affected_logs": [r["log_file"] for r in analysis_results if r.get("suspicious_patterns")]
            })
    ...
    def _check_integrity(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        total_modified = sum(len(result.get("recently_modified", [])) for result in integrity_results)
        total_suspicious = sum(len(result.get("suspicious_files", [])) for result in integrity_results)
        ...
        if total_modified > 5 or total_suspicious > 0:
            self.log_finding("integrity_violation", {
